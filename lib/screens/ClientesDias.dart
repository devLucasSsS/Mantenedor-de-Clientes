...
class ClientesDias extends StatefulWidget {
  String id;
  ClientesDias(this.id);
  @override
  State<ClientesDias> createState() => _ClientesDiasState();
}

class _ClientesDiasState extends State<ClientesDias> {
  String nombre = "";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      actions: [
        ...
        onSelected: (opcion) async{
          if(opcion == 'total'){
          final action = await AlertDialogsinfo.yesCancelDialog(context, 'Información', 'Dinero en la calle del día '+widget.id+' es: '+currencyFormatter.format(int.parse((getTotal().toString()))));
    ...
    body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('dias').doc(widget.id).collection('clientes').snapshots(),
          ...
          ListView.builder(
            ...
            itemBuilder: (context,index){
              ...
              if(nombre.isEmpty){
                ...
                subtitle:
                Text('Saldo Pendiente: '+ currencyFormatter.format(int.parse(deuda1))),
                ...
                onSelected: (opcion) async{
                  if(opcion == 'editar'){
                    ...
                    MaterialPageRoute route = MaterialPageRoute(builder: ((context) => DetalleCliente(widget.id,nombre, cl.id,1)));
                    ...
                    if(action == DialogsAction.yes){
                      setState(() {
                        tappedYes = true;
                        FireStoreService().borrar(widget.id,cl.id,1);
                    ...
                    var datos = cliente['nombre'] +'\nSaldo Pendiente Articulos de aseo: ' +currencyFormatter.format(int.parse(deuda1));
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          MaterialPageRoute route = MaterialPageRoute(builder: ((context) => AgregarCliente(widget.id,1)));
          Navigator.push(context, route);
        },
      ),
    );
